...
from pyzzz.dataset.marks import Lucy, Piper
...
def test_ellen():
    ...
    env2 = Env.from_full(build_test.full_lycaon())
    env.set_agent(1, env2.agent(0))

    # env3 = Env.from_full(build_test.full_soukaku())
    # env.set_agent(2, env3.agent(0))
...
def test_jane():
    ...
    env.set_agent(1, create_agent(name="Seth", skill_levels=SkillLevels(5)))
    env.set_agent(2, create_agent(name="Lucy", core_skill_atk=600))
    print(env)
    # env.enemy._defense_base = 54
...
def test_piper():
    env = Env.from_full(build_test.full_piper())
    # env.set_agent(1, create_agent(name="Seth", skill_levels=SkillLevels(5)))
    env.set_agent(2, create_agent(name="Lucy", core_skill_atk=1000))
    print(env)
    combo = [Piper.EX1]
